...
import { supabase, type Review, type GoogleReview } from '../lib/supabase';

// Fallback placeholder google review screenshots
...
export default function CustomerReviews() {
  ...
  const [googleReviews, setGoogleReviews] = useState<GoogleReview[]>(PLACEHOLDER_REVIEWS);
  ...
  useEffect(() => {
    ...
    ]).then(([{ data: reviewData }, { data: grData }]) => {
      setReviews(reviewData ?? []);
      const dbGr = grData ?? [];
      if (dbGr.length > 0) setGoogleReviews(dbGr);
      setLoading(false);
    });
  }, []);
